          ...
          if (extension.activeCalls.length){
            for (var call of extension.activeCalls){
              ...
              if (call.status == "NO CALL"){
                second = 0
                $("#call_"+call.sessionId).remove()
              }else if(call.status == "SETUP"){
                if ($("#call_"+call.sessionId))
                  //$("#extension_"+extension.id).empty()
                  $("#call_"+call.sessionId).remove()
                var html = makeActiveCallBlock(extension.name, call)
                $("#extension_"+extension.id).append(html)
              }else{
                //$("#extension_"+extension.id).empty()
                $("#call_"+call.sessionId).remove()
                var html = makeActiveCallBlock(extension.name, call)
...
function makeActiveCallBlock(name, call){
    //$("#call_"+call.sessionId).empty()
    var html = `<div id="call_${call.sessionId}" class='col-xs-12'>`
    ...
    html += `<div class='col-sm-1'>${call.status}</div>`
    html += `<div class='col-sm-1'>${call.duration}</div>`
    html += `<div class='col-sm-1'>${call.callRespondTime}</div>`
    html += `<div class='col-sm-1'>${call.callHoldDuration}</div>`
    html += `</div></div>`
    return html
  ...
  getting.done(function( res ) {
    if (res.status == "ok"){
      ...
      for (var ext of res.data){
        var html = `<div id='extension_${ext.id}' class='col-xs-12'>`
        html += `<div class='col-sm-2'><b>${ext.name}</b></div>`
...
function addExtension(){
  ...
  getting.done(function( res ) {
    if (res.status == "ok"){
      ...
      agentList.push(agent)
      var html = `<div id='extension_${ext.id}' class='col-xs-12'>`
      html += `<div class='col-xs-12'><div class='col-sm-2'><b>${ext.name}</b></div>`
      html += `<div class='col-sm-2'>--</div>`
      html += `<div class='col-sm-2'>--</div>`
      html += `<div class='col-sm-1'>--</div>`
      html += `<div class='col-sm-1'>--</div>`
